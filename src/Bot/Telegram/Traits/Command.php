	...
	PRIVATE FUNCTION _manga($args)
	{
        ...
        if (!empty($args)) {
            ...
            if (isset($st['entry']['id'])) {
                ...
                B::sendMessage($rep."\nBerikut ini adalah beberapa manga yang cocok dengan <b>{$args}</b>.\n\nKetik /idma [spasi] [id_manga] atau balas dengan id manga untuk menampilkan info manga lebih lengkap.", $this->room_id, $this->msg_id, ["reply_markup"=>"HTML", ["reply_markup"=>json_encode(["force_reply"=>true, "selective"=>true])]]);
            } else {
                B::sendMessage("Mohon maaf, anime \"{$args}\" tidak ditemukan !", $this->room_id, $this->msg_id);
            }
        } else {
